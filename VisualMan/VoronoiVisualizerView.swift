  ...
  var body: some View {
    GeometryReader { g in
      TimelineView(.animation) { timeline in
        Rectangle()
          .voronoiShader(time: time, smoothedBass: smoothedBass, smoothedMid: smoothedMid, smoothedHigh: smoothedHigh, width: Float(g.size.width), height: Float(g.size.height))
